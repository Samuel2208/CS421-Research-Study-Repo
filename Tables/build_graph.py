...
def create_f1_comparison_graph(csv_filepath):
    ...
    filtered_df['F1_Source_Clean'] = filtered_df['F1_Source'].apply(clean_source_name)
    ...
    for table in target_tables:
        ...
        for _, row in subset.iterrows():
            plt.annotate(
                row['F1_Source_Clean'],
                xy=(row['Window_Size'], row['Best_Macro_F1']),
                xytext=(0, 12),
    ...
    plt.title('Best Macro F1 Comparison: Gemma vs Qwen', fontsize=15, fontweight='bold')
    plt.xlabel('Window Size', fontsize=12)
    plt.ylabel('Best Macro F1', fontsize=12)
...
if __name__ == "__main__":
    create_f1_comparison_graph('best_window_size_metrics.csv')